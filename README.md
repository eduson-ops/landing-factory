...
1. **Выбор блоков.** Каталог из 35 шаблонов сгруппирован по 7 разделам. Клик по карточке выбирает её (фиолетовая рамка + галочка). Превью — живой `<iframe>` со scale 0.3125. Поиск по названиям и тегам.
2. **Пресеты.** Готовые наборы блоков одной кнопкой: «IT/ИИ-курс» (16), «Софт-навыки» (12), «Минимальный» (5), «Полный» (19).
3. **Тема.** `Light · Violet` (по умолчанию) или `Dark · Orange`. Блоки `hero-03-dark` и `stats-01-dark` имеют тему зашитую в файл — переключатель их не трогает.
4. **ТЗ.** Textarea внизу правой панели — туда продакт описывает курс. Кнопка «+ шаблон» сгенерирует каркас под выбранные блоки. ТЗ сохраняется в браузере (localStorage) и подставляется в промт автоматически.
5. **Режим промта:**
...
6. **Действия:** «Скопировать промт» / «Скачать .txt» / **«Посмотреть собранный лендинг»** (новая вкладка с device-toggle Desktop/Tablet/Mobile) / **«Поделиться выбором ссылкой»** (URL содержит выбранные блоки и тему — отправь коллеге, он откроет тот же набор).
7. **Состояние сохраняется** автоматически: при возврате на страницу выбор и ТЗ восстанавливаются.
8. **Защита от ошибок.** Если хотя бы один выбранный блок не загрузился — модал с предупреждением «N блоков не загрузились — всё равно сгенерировать?».

## Куда деплоить собранный лендинг

После того как Claude собрал `index.html` — есть три бесплатных способа быстро его опубликовать:

| Способ | Время | Что получаете |
|---|---|---|
| **[Netlify Drop](https://app.netlify.com/drop)** | 1 минута | Заходите без регистрации, перетаскиваете папку с `index.html` и `blocks/` — получаете URL вида `https://random-name.netlify.app/`. Можно потом подключить домен. |
| **[Vercel](https://vercel.com/new)** | 2 минуты | Логин через GitHub, `vercel --prod` из терминала, или drag-and-drop архива. URL `https://project.vercel.app/`. |
| **[GitHub Pages](https://pages.github.com/)** | 5 минут | Создаёте репо, пушите файлы, в Settings → Pages включаете деплой с `main` ветки. URL `https://<user>.github.io/<repo>/`. |

**Что нужно скопировать** в любой из них:
- `index.html` (собранный)
- папку `blocks/css/`
- папку `blocks/js/`
- папку `blocks/img/`

Это всё — ~50 KB кода + ~10 KB ассетов.

> **Tilda не подходит** — Tilda работает со своими zero-блоками, не с произвольным HTML. Если нужна Tilda — это отдельная задача переноса вёрстки в их редактор.
